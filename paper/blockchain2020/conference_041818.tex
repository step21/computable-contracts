...
% next two lines modified from outside
\usepackage{enumitem}
...
\author{\IEEEauthorblockN{1\textsuperscript{st} Some Author}
\IEEEauthorblockA{\textit{Certain Faculty} \\
\textit{Some Institute}\\
Someplace \\
some.email@someplace.com}
}
...
% maybe shorten primer to just what is important for paper?
...
%Under the heading of ‘AI \& Law’ we find more modern machine learning approaches  towards understanding textual law or loads of cases as well as automatic processing of forms and legal reasoning, from very simple to very complex and layered systems.
...
%An alternative project with similar goals is LKIF (Legal Knowledge Interchange Format)  Both projects have similar goals but different groups and stakeholders developed them. Similarly, RuleML also defined a markup language (based on XML) and activities, initiatives and technical standards, though relating more generally to rules and not law or contracts per se.
%A ‘smaller’ system like this would be something like Legal Markdown  or Cicero (by Accord Project) , as these are much more limited in scope, focusing on display easy templating.  
...
Specifically, Defeasible logic \cite{nute_defeasible_1993,garciaDefeasibleLogicProgramming2002} was developed as an extension to simple argumentative logic. More widespread and complete languages like Prolog \cite{SWIPrologManual} %and ReasonML \cite{ReasonReasonLets}  
...
%Either of these has been used and reasearched for judicial / legal argumentation and legislation more generally or contract specifically.
%Part of logic-based languages, expert systems (a precursor what today is known as 'AI’ or ‘ML’) based on these (or expert systems based on other languages) are ontologies and knowledge bases.  Ontologies  are ‘explicit format specifications of the terms in the domain and the relations among them’  and a set of instantiated ontological classes constitutes a knowledge base that can be used by logic-based languages or more generally expert systems and similar instruments.
...
%Furthermore, similar metadata formats are also used for other media such as images and video. An open source project that tried to so sth similar was elog.io , then followed up by videoroot where the attempt was to use image search instead as metadata is stripped much more easily. 
...
% hackathon projects such as ‘Dickstinction’  (compilation of a pdf for reporting ‘dickpics’ or other unwanted images to the authorities and contributoragreements.org  where open source developers and projects can use the generator to build their own custom contributor license agreement. (which is used to protect the project, enable license changes or enable stewardship by a foundation such as FSFE).
...
%also will potentially be thought elsewhere. 
...
%there is still some hierarchy, 
%or at least it can be derived in a specific case, such as that text and executable code are probably always higher in rank than intent signified by marketing communication.
...
% old While the analogy breaks down a bit with regard to legal sources, as they are more of a hierarchy, there is also not always a clear hierarchy, as higher norms will generally be more abstract and describe a higher level, which then need other levels or indirect communication for interpretation, so the general analogy holds and is present in both public and private law.
...
From a legal point of view, depending on jurisdiction there is a definite distinction between license and contract, especially in the United States. % dist btw license contract, cite contractual nature of license 
...
Compared to the past work, this license example is extended to make for a more realistic and harder testcase. %The testcase is better and harder because it also is compared against SCT  systems and the Lexon NLP-like system.
%It is more realistic because it was compared to realistic licenses and adapted accordingly.
...
%In case these features make it harder or impossible to do something, this will also be described.
...
% after briefly having introduced smart contract technology and its peculiarities, also highlighting changes to the older work. 
...
% line below also left out  before, but maybe keep?
%Thirdly, the use of a smart contract templating system as categorized by Meng Wong  and others is demonstrated, and the current status of SCT systems is assessed briefly.
%\begin{figure}
...
Compared to the previous work, \cite{governatoriLegalContractsImperative2018} this example adds important features that make it an actual license. %Most importantly,
...
%The asset that is licensed is defined or at least can defined. Furthermore, it was important to clarify the relation between commenting and publication, and how this relates to the possibility of the licensee being commissioned by the licensor.
...
%realistic but not too hard starting point for our experiment of examining different license implementations, as he claims computable contracting is limited by possibilities of automated assessments, but which he claims go ‘far enough’ more often than not. So ‘good enough'. For now.
...
The solidity language was established by the Ethereum project. \cite{gavinwoodEthereumSecureDecentralised2013} It is an imperative %and compiled 
...
Prolog is here used as an example for various kinds of attempts at using primarily logic-based languages for contract like systems or generally reasoning about law and contracts. Reasoning is here understood as making deductions based on a set of inputs.
%While the use of defeasible logic/reasoning can be very useful for legal reasoning/logic-based evaluations, its tooling and solving infrastructure is not that widespread, and can be unnecessarily difficult to acquire. 
Furthermore, the syntax and structure of Prolog is more easily acquired and tested.% For comparison, one line of Defeasible Logic is compared with one line of Prolog.
...
While Prolog is not as tailored to be defeasible and can in principle have unclear states, it otherwise supports everything that DL supports and even much more. \cite{morelliralphPROLOGFactsRules} %Most importantly however, it is much more accessible, meaning that everyone can test the provided listing in seconds in a web browser or on a desktop computer. While there are many implementations available, because of availability and widespread use, SWI-Prolog  was used.
The main parts of Prolog used in this research are facts ‘exists(paper).’, which means paper exists, rules denoted by ‘:-’ which evaluate the left statement to true if the conditions on the right hand side below match, and variables, denoted by every term starting with a capital letter. As the Prolog interpreter, similar to a DL implementation % or many similar systems 
...
\subsubsection{Implementation}
The Prolog implementation of the textual license example mentioned above can be found below. % Here, the license is briefly explained and evaluated.
...
%a logic-based system was said to have in earlier work.
...
\subsubsection{Technology} % also mention gitlab/source somewhere?
...
%\end{example}
%\end{algorithmic}
%This example demonstrates, that f
...
\lstinputlisting[language=Lexon, caption={This is the Lexon version (based on version 0.2.12).}]{license_eval.lex}
...
Prolog was looked at, a widely available, accessible and extensible, logic-based language. %Prolog was chosen as opposed to more specific offshoots, as it is much more accessible and reproduceable, and the high cost of accessing a niche language such as DL or DR-Prolog or similar outweighed their benefits. 
...
%\begin{lstlisting}
%
%is_commissioned(license) vs commissioned(license)
%\end{lstlisting}
%\end{algorithmic}
...
%it does not look entirely natural, in that the way clauses are described, requires some change of thinking for non-programmers However, these can still be easily understood when read and this will probably also improve with time or when used with another normative system as a foundation. 
...
In this paper, an example license was presented and implemented in four different tech stacks. %Towards understanding of this exercise and the needs of a contract, a state of things of computable law and the legal contractual landscape was given.
...
and Lexon. The main characteristics for examination were ‘meaning and spirit of the contract’, syntax and semantics (of the original license), functionality and looks. %/readability. 
Based on these factors newer contestants such as Lexon %(barring new developments from compk/CSL or similar)
is the most promising candidate right now, as they are their core not too complicated and reliant on things like statistical reasoning, but at the same time still very much still look like natural language and are also very functional at the same time.% If they then would be combined with contract management tooling, contract templating and statistical approaches, the result would likely be stellar.

%\section*{Acknowledgment}

%Special thanks to Prof. Dr. Giovanni Sartor at EUI for his support, Dr. Fatma Sayed at EUI for even more support and finally but not least Dr. Henning Diedrich from Lexon Foundation.


\bibliographystyle{IEEEtran}
\bibliography{blockchain2020.bib}{}
